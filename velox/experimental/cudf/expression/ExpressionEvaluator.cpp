...
ColumnOrView FunctionExpression::eval(
  ...
  if (function_) {
    ...
    // The batch row count, threaded into eval so a function with no input
    // column to borrow a length from can still size its output. Taken from the
    // first subexpression result, or from the input when there are no
    // subexpressions (zero-column projections fall back to CPU and empty
    // batches short-circuit upstream).
