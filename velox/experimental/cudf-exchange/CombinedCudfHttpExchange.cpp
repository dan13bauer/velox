...
CombinedCudfHttpExchange::CombinedCudfHttpExchange(
  ...
  if (exchangeClientFacade) {
    ...
    exchangeClient_ = std::make_shared<ExchangeClientFacade>(std::move(client), nullptr);
...
RowVectorPtr CombinedCudfHttpExchange::getOutputFromPages(
  ...
  auto cudfFromVelox = std::make_shared<facebook::velox::cudf_velox::CudfFromVelox>(
