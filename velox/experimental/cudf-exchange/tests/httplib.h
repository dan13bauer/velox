...
class Server {
 ...
 private:
  ...
  void output_pre_compression_log(const Request& req, const Response& res)
      const;
...
class Result {
 public:
  ...
  std::string get_request_header_value(
      ...
      const char* def = "",
      size_t id = 0) const;
  size_t get_request_header_value_u64(
      const std::string& key,
      size_t def = 0,
      size_t id = 0) const;
...
inline bool is_numeric(const std::string& str) {
  return !str.empty() &&
      std::all_of(str.cbegin(), str.cend(), [](unsigned char c) {
...
inline size_t Request::get_header_value_u64(
    const std::string& key,
    size_t def,
    size_t id) const {
...
inline size_t Response::get_header_value_u64(
    const std::string& key,
    size_t def,
    size_t id) const {
  return detail::get_header_value_u64(headers, key, def, id);
...
inline size_t Result::get_request_header_value_u64(
    const std::string& key,
    size_t def,
    size_t id) const {
...
std::string encode_query_component(
    const std::string& component,
    bool space_as_plus = true);
std::string decode_query_component(
    const std::string& component,
...
class gzip_decompressor final : public decompressor {
 public:
  ...
  bool decompress(const char* data, size_t data_length, Callback callback)
      override;
...
inline bool zstd_compressor::compress(
    const char* data,
    size_t data_length,
    bool last,
    Callback callback) {
...
inline bool zstd_decompressor::decompress(
    const char* data,
    size_t data_length,
    Callback callback) {
  std::array<char, CPPHTTPLIB_COMPRESSION_BUFSIZ> buff{};
...
template <typename T>
inline ReadContentResult read_content_chunked(
    Stream& strm,
    T& x,
    size_t payload_max_length,
    ContentReceiverWithProgress out) {
...
inline bool parse_accept_header(
    const std::string& s,
    std::vector<std::string>& content_types) {
  content_types.clear();
...
inline std::string encode_query_component(
    const std::string& component,
    bool space_as_plus) {
  std::string result;
  result.reserve(component.size() * 3);
...
inline std::string decode_query_component(
    const std::string& component,
    bool plus_as_space) {
  std::string result;
  result.reserve(component.size());
...
inline std::string Request::get_trailer_value(const std::string& key, size_t id)
    const {
...
inline std::string Request::get_param_value(const std::string& key, size_t id)
    const {
...
inline std::string MultipartFormData::get_field(
    const std::string& key,
    size_t id) const {
...
inline FormData MultipartFormData::get_file(const std::string& key, size_t id)
    const {
...
inline std::string Response::get_trailer_value(
    const std::string& key,
    size_t id) const {
  auto rng = trailers.equal_range(key);
  auto it = rng.first;
...
inline void SocketStream::get_remote_ip_and_port(std::string& ip, int& port)
    const {
  return detail::get_remote_ip_and_port(sock_, ip, port);
}

inline void SocketStream::get_local_ip_and_port(std::string& ip, int& port)
    const {
...
inline void Server::output_pre_compression_log(
    const Request& req,
...
inline void Server::output_error_log(const Error& err, const Request* req)
    const {
  if (error_logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
...
inline void ClientImpl::output_log(const Request& req, const Response& res)
    const {
  if (logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
    logger_(req, res);
...
inline void ClientImpl::output_error_log(const Error& err, const Request* req)
    const {
  if (error_logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
    error_logger_(err, req);
...
inline void SSLSocketStream::get_remote_ip_and_port(std::string& ip, int& port)
    const {
  detail::get_remote_ip_and_port(sock_, ip, port);
}

inline void SSLSocketStream::get_local_ip_and_port(std::string& ip, int& port)
    const {
...
inline bool SSLClient::check_host_name(const char* pattern, size_t pattern_len)
    const {
